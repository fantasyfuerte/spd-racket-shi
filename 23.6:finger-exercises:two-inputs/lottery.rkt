...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname lottery) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (random-pick l) (list-ref l (random (length l))))
  ...
  (cond
    ...
    [else
     (local ((define (non-same? names l)
               (cond
                 [(empty? l) #true]
                 [(string=? (first names) (first l)) #false]
                 [else (non-same? (rest names) (rest l))])))
       (append
